...
aws.config.update({
  accessKeyId: "AKIAJPW5LUOXBCUPKKDQ",
  secretAccessKey: "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"
})
const s3 = new aws.S3()
const Storage = s3Storage({
  s3,
  Bucket: 'evergreen-dev',
  ACL: 'public-read',
});
    ...
    if (!user_id) {
      ...
      await DataField.findAll({ where: { id: user_id } }).then(async response => {
        ...
        let testing = response.map(item => {
          pageData = item.dataValues.page_url_check.push(page_url_check)
          idData = item.dataValues.page_id.push(page_id)
          return item
        })
  ...
  async updatePromoRoute(req, res) {
    ...
    if (type === "offer") {
      ...
      await Offer.findAll({ where: { id: user_id } }).then(async response => {
        // console.log("resp",response)
        let testing = response.map(item => {
          promoId = item.dataValues.custom_page_promo_ids.push(custom_page_promo_ids)
          localId = item.dataValues.custom_page_local_ids.push(custom_page_local_ids)
          promo_route = item.dataValues.custom_page_promo_routes.push(custom_page_promo_routes)
          return item
        })
        // console.log("resp", testing[0].dataValues)
        await testing.map(async item => {
